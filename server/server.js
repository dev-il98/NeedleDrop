...
// Only internal paths that actually initiate a Spotify login. Prevents an
// open-redirect via a crafted ?returnTo= value, and any value outside this
// list silently falls back to "/host" — exactly today's behavior.
const RETURN_TO_ALLOWLIST = ["/host", "/join"];
...
app.get("/auth/login", (req, res) => {
  ...
  // Where to send the browser back to after Spotify's OAuth flow. Defaults
  // to /host (existing behavior, unchanged) unless a recognized page
  // explicitly asked to return elsewhere — currently only /join, so a
  // player connecting Spotify from the join flow lands back on /join
  // instead of being dumped on /host.
  const requestedReturnTo = req.query.returnTo;
  const returnTo = RETURN_TO_ALLOWLIST.includes(requestedReturnTo) ? requestedReturnTo : "/host";
...
app.get("/auth/callback", async (req, res) => {
  ...
  const returnTo = RETURN_TO_ALLOWLIST.includes(req.cookies.oauth_return_to)
